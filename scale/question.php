...
class qtype_musictheory_scale_write extends qtype_musictheory_question implements qtype_musictheory_subtype {
    ...
    public function is_complete_response(array $response) {
        if ($this->musictheory_scaletype == 'melodic') {
            ...
            $regex = '/^([A-G](n|\#|b|x|bb)[2-6],){7}([A-G](n|\#|b|x|bb)[2-6]){1}$/';
        }
    ...
    public function get_validation_error(array $response) {
        if ($this->musictheory_scaletype == 'melodic') {
            $incompleteregex = '/^(([A-G](n|\#|b|x|bb)[2-6])?,?){0,15}$/';
            $incompleteregex2 = '/^(([A-G](n|\#|b|x|bb)[2-6]),){0,13}([A-G](n|\#|b|x|bb)[2-6]){1}$/';
        } else {
            $incompleteregex = '/^(([A-G](n|\#|b|x|bb)[2-6])?,?){0,8}$/';
            $incompleteregex2 = '/^(([A-G](n|\#|b|x|bb)[2-6]),){0,6}([A-G](n|\#|b|x|bb)[2-6]){1}$/';
        }
    ...
    public function get_question_text() {
        ...
        $scale = get_string('note' . $this->musictheory_givennoteletter, 'qtype_musictheory');
        $scale .= $acc;
